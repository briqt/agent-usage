...
- `internal/collector` — Source-specific parsers. Each collector implements `Scan()` which walks session directories and calls `processFile()` for incremental parsing. File offsets tracked in `file_state` table to avoid re-reading. `claude.go`/`claude_process.go` is the reference implementation for adding new sources. Collectors must normalize token fields to match the non-overlapping semantics defined below. Collectors also extract individual user prompt events (with timestamps) into the `prompt_events` table for time-accurate prompt counting. The Kiro collector (`kiro.go`/`kiro_process.go`) supports **two data sources** simultaneously: (1) a SQLite database (`~/.local/share/kiro-cli/data.sqlite3`) containing `conversations_v2` with per-request metadata, and (2) JSON session files (`~/.kiro/sessions/cli/*.json` + companion `*.jsonl`) with aggregated turn metadata. `Scan()` auto-detects each path type and routes accordingly. Kiro does not expose actual token counts, so tokens are **estimated**: input from `context_usage_percentage × context_window_tokens`, output from `response_size / 4` (SQLite) or CJK-aware character heuristics on JSONL content (JSON source). Known limitations: (1) subagent sessions (null `session_state` in JSON) cannot be tracked; (2) token counts are estimates, not exact values; (3) the two sources have disjoint session IDs — no overlap or dedup concern. The Pi collector (`pi.go`/`pi_process.go`) shares the same JSONL format as OpenClaw (same underlying framework). It tracks `model_change` entries for mid-session model switching and derives project names from the session CWD (with workspace slug as fallback). Directory structure: `~/.pi/agent/sessions/<workspace-slug>/<file>.jsonl`. The Hermes collector (`hermes.go`/`hermes_process.go`) reads Hermes Agent's SQLite `state.db` directly. Auto-discovers `state.db` at the base path and `profiles/*/state.db` for multi-profile setups. Token semantics match directly (input_tokens is already non-cached). Prompt events are extracted from the `messages` table (`role='user'`). Because a database has no byte offset to resume from, incrementality works differently than for the file-based collectors, on two levels: (1) each database is gated on a change fingerprint stored in `file_state` — size and mtime of both the database **and its `-wal`** (in WAL mode a write lands in the log and leaves the main file untouched, so watching only the main file misses updates), so an idle instance costs two `stat` calls per interval instead of a full table scan; (2) when a database has changed, usage records and sessions are replaced within that database's **project scope only** (`DeleteBySourceProject`), never source-wide — a source-wide delete combined with skipping unchanged databases would discard the records of every database that got skipped. The replace is needed because Hermes accumulates tokens on the session row in place and the `usage_records` dedup index covers the token columns, so a grown session would land as an additional row rather than replacing the previous one. Prompt events are exempt from the replace: they are appended from a rowid watermark (`messages.id > ?`, an append-only `INTEGER PRIMARY KEY`) also kept in `file_state`, and re-scanning already-consumed rows cannot regenerate them. The watermark is taken from `MAX(id)` rather than from the returned rows, so messages excluded by the `role`/`timestamp` filter are stepped over instead of being re-examined on every scan forever.
- `internal/storage` — SQLite layer. `sqlite.go` has schema + versioned migrations (tracked via `meta` table with `migration_{id}` keys, each runs once), `queries.go` handles writes, `api.go` handles reads, and `billing.go` performs deterministic recalculation. Key tables: `usage_records` (per-API-call token/cost/provenance data), `sessions`, `prompt_events`, `pricing`, and `file_state`. Opened with `synchronous=NORMAL`, which is safe under WAL (a crash can lose the tail of the last transaction but cannot corrupt the database) and drops an fsync per commit — the collectors commit continuously. `DB.Checkpoint()` runs `wal_checkpoint(TRUNCATE)` on a timer from `main.go`: under a steady write load SQLite's automatic checkpointing can be starved indefinitely, and an oversized WAL makes every read walk it before reaching the main file. `RecalcPendingCosts` runs after every collector scan and filters on `priced_at IS NULL`, served by the partial index `idx_usage_unpriced`. That index is created **after** the `billingAlters` block, not inside the main schema statement: on a database predating the billing columns, `CREATE TABLE IF NOT EXISTS` is a no-op and `priced_at` only exists once the `ALTER` has run, so creating the index earlier fails with "no such column" and takes the whole migration — and startup — with it.
