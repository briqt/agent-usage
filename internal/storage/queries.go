...
//
// The first group of fields is parser context for JSONL sources whose session
// metadata only appears at the top of the file. The DB* fields are the change
// fingerprint for SQLite-backed sources (Hermes), which have no byte offset to
// resume from and instead skip untouched databases outright.
type FileScanContext struct {
	...
	// Change fingerprint for SQLite sources. The -wal file must be part of it:
	// in WAL mode a write lands in the log and leaves the main database file's
	// size and mtime untouched, so watching only the main file misses updates.
	DBMTimeNano  int64 `json:"db_mtime_nano,omitempty"`
	WALSize      int64 `json:"wal_size,omitempty"`
	WALMTimeNano int64 `json:"wal_mtime_nano,omitempty"`
}
...
// DeleteBySourceProject removes usage_records and sessions for one
// (source, project) pair, leaving prompt_events intact.
//
// Session-level sources (Hermes) re-derive a session's totals on every scan
// because tokens accumulate in place, and the usage_records dedup index covers
// the token columns — so a grown session would otherwise land as a second row
// rather than replacing the first. Scoping the delete to a single project keeps
// each backing database independent, so skipping an unchanged one never discards
// another's records.
//
// prompt_events are deliberately excluded: they are appended incrementally from a
// rowid watermark and cannot be regenerated by a re-scan of already-seen rows.
func (d *DB) DeleteBySourceProject(source, project string) error {
	d.mu.Lock()
	defer d.mu.Unlock()
	tx, err := d.db.Begin()
	if err != nil {
	...
	defer tx.Rollback()
	if _, err := tx.Exec("DELETE FROM usage_records WHERE source=? AND project=?", source, project); err != nil {
		return err
	}
	if _, err := tx.Exec("DELETE FROM sessions WHERE source=? AND project=?", source, project); err != nil {
		return err
	}
	return tx.Commit()
}
